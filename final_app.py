...
# Display the main app name
st.title("Query Exploration App")

# Display the choice options in the sidebar
option = st.sidebar.selectbox("Choose an app", ("Query by Image", "Query by Text"))
...
def Image_Match():
    ...
    st.subheader("Image Query Search")
    # st.write("Explore Similar Images")
    ...
    def search_similar_images(user_uploaded_image, image_style_embeddings, images, max_results=10):
        ...
        st.subheader("Most similar images to your query:")
        for i, (image_path, distance) in enumerate(sorted_neighbors[:max_results]):
            st.image(images[image_path], caption=f"Distance: {distance}", use_column_width=True)
...
def Image_Search_by_Text():
    ...
    # Streamlit app
    st.subheader("Text Query Search")
...
if option == "Query by Image" :
    Image_Match()
elif option == "Query by Text":
